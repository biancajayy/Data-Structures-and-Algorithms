 ...
 * @version 1.0
 * @userid YOUR USER ID HERE bjayaraman0
 * @GTID YOUR GT ID HERE 903754012
 *
...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        double futureSize = (size + 1) / (double) table.length;
        //resize case
        if (futureSize >= MAX_LOAD_FACTOR) {
        ...
        if (table[index] == null) {
            ...
            //add to head
            ExternalChainingMapEntry<K, V> newHead = new ExternalChainingMapEntry<>(key, value, table[index]);
            table[index] = newHead;
        }
        size++;
        return null;
    ...
    public V remove(K key) {
        ...
        int index = Math.abs(key.hashCode() % table.length);
        if (table[index] == null) {
            throw new NoSuchElementException("Key does not exist within "
                    + "the map.");
        } else {
            ExternalChainingMapEntry<K, V> curr = table[index];
            if (curr.getKey().equals(key)) {
                ExternalChainingMapEntry<K, V> newCurr = curr.getNext();
                V deletedValue = curr.getValue();
                curr.setNext(null);
                size--;
                table[index] = newCurr;
                return deletedValue;
                ...
                while (curr.getNext() != null) {
                    if (curr.getNext().getKey().equals(key)) {
                        ExternalChainingMapEntry<K, V> deletedNode = curr.getNext();
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("The resize array can not have "
                    + "a length shorter than the initial hash map.");
        }
        ...
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null) {
                ...
                while (curr != null) {
                    ...
                    ExternalChainingMapEntry<K, V> reHashNode
                            = new ExternalChainingMapEntry<>(curr.getKey(), curr.getValue(), newTable[index]);
                    newTable[index] = reHashNode;
                    curr = curr.getNext();
                }
            }
        }
